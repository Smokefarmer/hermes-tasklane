...
## 8. Tester credentials (conversational intake)

A TESTER job — the optional pipeline `test` stage, or `test_deployment` — verifies
the app by **actually running it**. To log in it needs real test credentials, which
live in a per-project mode-600 secret file; the tester only ever sees the KEY NAMES.

**Before you launch a tester or a `…,test` pipeline**, check what's stored:

```
list_project_secret_keys(repo="/home/me/dev/api")   # → {"key_names": [...]}  (NAMES only)
```

If that list has **no `TEST_USER` / `TEST_PASSWORD`** for the project, ask the human:

> "Is there a login? Please give me a demo/test user so I can verify the changes for you"

Then store the answer — once — and continue:

```
set_project_secrets(repo="/home/me/dev/api",
                    secrets={"TEST_USER": "...", "TEST_PASSWORD": "...", "STAGING_URL": "..."})
```

Rules, non-negotiable:

- **NEVER repeat the values back** in chat, in a job brief, or in any summary. They
  are write-only to you; the server keeps them in a mode-600 file and surfaces only
  the names. Confirm with the names ("stored TEST_USER, TEST_PASSWORD"), not values.
- Keys must be `SCREAMING_SNAKE_CASE`; values non-empty, ≤4096 chars, ≤50 per call.
- `set_project_secrets` merges (re-send one key to rotate it). Remove a stale key with
  `delete_project_secret(repo, key)`; re-list with `list_project_secret_keys`.
- The repo must be registered in the project registry (a `projects:` entry whose
  `repo:` matches). The first store auto-creates `~/.tasklane/secrets/<project>.env`
  and links the registry to it.
