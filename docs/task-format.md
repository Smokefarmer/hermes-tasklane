...
## Operator metadata generated by wave enqueue

Wave-generated task files remain ordinary task files and preserve the existing frontmatter format. After `wave-runner --enqueue` or `plan-wave --enqueue` syncs those files into JobStore records, Tasklane writes an optional lane-plan artifact at:

```text
~/.local/share/hermes-tasklane/lane-plans/<wave_id>.json
```

The artifact is not required for job execution. It is an operator aid used by `inspect` and the dashboard when present. It records `schema_version`, `wave_id`, `project`, `repo_path`, `base_branch`, `artifact_status`, and per-lane `delivery_group`, `lane_id`, `branch`, `issue_numbers`, `task_uids`, `job_ids`, `implementation_task_uids`, `implementation_job_ids`, `final_pr_task_uid`, and `final_pr_job_id`.
